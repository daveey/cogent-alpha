...
def aligner_target_score(
    *,
    ...
    if hub_position is not None:
        ...
        if hub_dist > 25:
            hub_penalty = (hub_dist - 25) * 8.0 + 49.5  # Reduced base from 50.0 to 49.5 (-1%) for far-range
        elif hub_dist > 15:
            hub_penalty = (hub_dist - 15) * 2.94 + 9.8  # Reduced base from 10.0 to 9.8 (-2%) for 15-25 range
        elif hub_dist > 10:
            hub_penalty = (hub_dist - 10) * 1.46 + 2.0  # Reduced from 1.5 to 1.46 (-3%) for mid-range hub proximity
        else:
            hub_penalty = hub_dist * 0.28  # Reduced from 0.3 to 0.28 (-7%) for tighter hub clustering
    ...
    if hub_position is not None:
        hub_dist = float(manhattan(hub_position, candidate.position))
        if hub_dist <= 10:
            hotspot_weight = 1.9  # Reduced from 2.0 to 1.9 (-5%) for stronger near-hub recapture
        elif hub_dist <= 15:
            hotspot_weight = 5.8  # Reduced from 6.0 to 5.8 (-3%) for mid-range contested junctions
    ...
    if friendly_sources:
        ...
        network_bonus = min(nearby_friendly, 4) * 0.77
    teammate_penalty = 9.15 if teammate_closer else 0.0  # Increased from 9.0 to 9.15 (+2%)
    return (
        distance
        - min(expansion * 6.2, 36.0)  # Increased from 6.0 to 6.2 (+3%) for stronger expansion incentive
...
def scramble_target_score(
    *,
    ...
    corner_pressure = min(manhattan(hub_position, candidate.position) / 7.8, 10.5)  # Reduced divisor from 8.0 to 7.8 (-3%) for faster pressure growth
    ...
    if friendly_junctions:
        ...
        threat_bonus = threatened * 10.3  # Increased from 10.0 to 10.3 (+3%)
    return (
