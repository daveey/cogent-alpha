    ...
    *,
    ...
    if hub_position is not None:
        ...
        if hub_dist > 25:
            ...
            hub_penalty = (hub_dist - 15) * 2.92 + 10.0  # Reduced from 2.93 for mid-far hub preference
        elif hub_dist > 10:
            hub_penalty = (hub_dist - 10) * 1.44 + 2.0  # Reduced from 1.45 for mid-range hub preference
        else:
            hub_penalty = hub_dist * 0.26  # Reduced from 0.27 for slightly stronger near-hub preference
    ...
    hotspot_weight = 10.3  # Reduced from 10.4 for far-range contested junction flexibility
    if hub_position is not None:
        hub_dist = float(manhattan(hub_position, candidate.position))
        if hub_dist <= 10:
            hotspot_weight = 1.86  # Reduced from 1.88 for stronger near-hub recapture incentive
        elif hub_dist <= 15:
            hotspot_weight = 5.5  # Reduced from 5.6 for mid-range contested junction flexibility
    ...
    if friendly_sources:
        ...
        network_bonus = min(nearby_friendly, 4) * 0.96  # Increased from 0.94 for stronger consolidation incentive
    teammate_penalty = 6.2 if teammate_closer else 0.0  # Reduced from 6.3 for more flexible coordination
    return (
        distance
        - min(expansion * 6.55, 36.0)  # Increased from 6.5 for stronger expansion incentive
        + enemy_aoe * 8.7  # Reduced from 8.8 for more aggressive contested territory targeting
...
def scramble_target_score(
    *,
    ...
    if friendly_junctions:
        ...
        threat_bonus = threatened * 11.0  # Increased from 10.9 for stronger network defense
    return (
        distance - blocked_neutrals * 8.9 - corner_pressure - threat_bonus,  # Increased from 8.8 for stronger expansion blocking
