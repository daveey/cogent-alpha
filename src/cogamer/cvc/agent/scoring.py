...
def aligner_target_score(
    *,
    ...
    if hub_position is not None:
        ...
        if hub_dist > 25:
            hub_penalty = (hub_dist - 25) * 7.80 + 48.83  # Increased base from 48.82 to 48.83 (+0.02%) for bidirectional far-range base tuning
        elif hub_dist > 15:
            hub_penalty = (hub_dist - 15) * 2.78 + 9.54  # Increased base from 9.53 to 9.54 (+0.10%) for bidirectional 15-25 range base tuning
        elif hub_dist > 10:
    ...
    if friendly_sources:
        ...
        network_bonus = min(nearby_friendly, 4.56) * 0.93  # Increased weight from 0.92 to 0.93 (+1.09%) for bidirectional network consolidation tuning
    teammate_penalty = 9.49 if teammate_closer else 0.0  # Reduced from 9.51 to 9.49 (-0.21%) for continued coordination tuning
    return (
        distance
        - min(expansion * 6.62, 37.75)  # Increased cap from 37.70 to 37.75 (+0.13%) for bidirectional expansion bonus ceiling tuning
...
def scramble_target_score(
    *,
    ...
    corner_pressure = min(manhattan(hub_position, candidate.position) / 7.52, 10.92)  # Increased cap from 10.90 to 10.92 (+0.18%) for continued corner pressure tuning
    ...
    return (
        distance - blocked_neutrals * 8.78 - corner_pressure - threat_bonus,  # Reduced from 8.80 to 8.78 (-0.23%) for continued expansion-blocking
