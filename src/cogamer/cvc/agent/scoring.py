...
def aligner_target_score(
    *,
    ...
    if friendly_sources:
        ...
        network_bonus = min(nearby_friendly, 4.42) * 0.86  # Increased cap from 4.4 to 4.42 (+0.45%) for continued denser network clustering
    teammate_penalty = 9.37 if teammate_closer else 0.0  # Increased from 9.35 to 9.37 (+0.21%) for continued coordination tuning
    return (
        distance
        - min(expansion * 6.48, 37.4)  # Increased cap from 37.35 to 37.4 (+0.13%) for continued higher expansion bonus ceiling
        + enemy_aoe * 10.62  # Increased from 10.6 to 10.62 (+0.19%) for continued enemy avoidance tuning
...
def scramble_target_score(
    *,
    ...
    corner_pressure = min(manhattan(hub_position, candidate.position) / 7.64, 10.76)  # Reduced divisor from 7.66 to 7.64 (-0.26%) for continued faster pressure growth
    ...
    if friendly_junctions:
        ...
        threat_bonus = threatened * 10.46  # Increased from 10.44 to 10.46 (+0.19%) for continued defensive priority tuning
    return (
        distance - blocked_neutrals * 8.66 - corner_pressure - threat_bonus,  # Increased from 8.64 to 8.66 (+0.23%) for continued expansion-blocking
