...
def aligner_target_score(
    *,
    ...
    if hub_position is not None:
        ...
        if hub_dist > 25:
            hub_penalty = (hub_dist - 25) * 7.86 + 48.89  # Reduced base from 48.90 to 48.89 (-0.02%) for continued far-range base tuning
        elif hub_dist > 15:
    ...
    if hub_position is not None:
        hub_dist = float(manhattan(hub_position, candidate.position))
        if hub_dist <= 10:
            hotspot_weight = 1.72  # Reduced from 1.74 to 1.72 (-1.15%) for continued near-hub recapture tuning
        elif hub_dist <= 15:
            hotspot_weight = 5.54  # Reduced from 5.56 to 5.54 (-0.36%) for continued mid-range contested junction tuning
