...
def aligner_target_score(
    *,
    ...
    if hub_position is not None:
        ...
        if hub_dist > 25:
            hub_penalty = (hub_dist - 25) * 7.89 + 48.92  # Reduced base from 48.93 to 48.92 (-0.02%) for continued far-range base tuning
        elif hub_dist > 15:
            hub_penalty = (hub_dist - 15) * 2.87 + 9.63  # Reduced multiplier from 2.88 to 2.87 (-0.35%) for continued 15-25 range multiplier tuning
        elif hub_dist > 10:
            hub_penalty = (hub_dist - 10) * 1.40 + 1.90  # Reduced multiplier from 1.41 to 1.40 (-0.71%) for continued 10-15 range multiplier tuning
        else:
            hub_penalty = hub_dist * 0.270  # Reduced from 0.271 to 0.270 (-0.37%) for continued tighter hub clustering tuning
    ...
    hotspot_weight = 11.66  # Reduced from 11.68 to 11.66 (-0.17%) for continued far-range contested junction tuning
    if hub_position is not None:
        hub_dist = float(manhattan(hub_position, candidate.position))
        if hub_dist <= 10:
            hotspot_weight = 1.78  # Reduced from 1.8 to 1.78 (-1.11%) for continued near-hub recapture tuning
        elif hub_dist <= 15:
            hotspot_weight = 5.60  # Reduced from 5.62 to 5.60 (-0.36%) for continued mid-range contested tuning
    ...
    if friendly_sources:
        ...
        network_bonus = min(nearby_friendly, 4.4) * 0.86  # Increased weight from 0.85 to 0.86 (+1.18%) for continued stronger network consolidation
