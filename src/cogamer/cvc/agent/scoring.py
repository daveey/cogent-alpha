    ...
    *,
    ...
    if hub_position is not None:
        ...
        if hub_dist > 25:
            ...
            hub_penalty = (hub_dist - 15) * 2.78 + 9.54  # Reduced multiplier from 2.79 to 2.78 (-0.36%) for continued 15-25 range multiplier tuning
        elif hub_dist > 10:
            hub_penalty = (hub_dist - 10) * 1.31 + 1.82  # Increased base from 1.81 to 1.82 (+0.55%) for continued 10-15 range base penalty tuning
        else:
            hub_penalty = hub_dist * 0.262  # Increased from 0.261 to 0.262 (+0.38%) for continued tighter hub clustering tuning
    ...
    hotspot_weight = 11.52  # Reduced from 11.54 to 11.52 (-0.17%) for continued far-range contested junction tuning
    if hub_position is not None:
        hub_dist = float(manhattan(hub_position, candidate.position))
        if hub_dist <= 10:
            hotspot_weight = 1.64  # Reduced from 1.66 to 1.64 (-1.20%) for continued near-hub recapture tuning
        elif hub_dist <= 15:
            hotspot_weight = 5.46  # Reduced from 5.48 to 5.46 (-0.37%) for continued mid-range contested junction tuning
    hotspot_penalty = min(hotspot_count, 3.32) * hotspot_weight  # Reduced cap from 3.34 to 3.32 (-0.60%) for continued contested junction penalty cap tuning
    ...
    if friendly_sources:
        ...
        network_bonus = min(nearby_friendly, 4.56) * 0.93  # Reduced cap from 4.58 to 4.56 (-0.44%) for continued denser network clustering
    teammate_penalty = 9.51 if teammate_closer else 0.0  # Reduced from 9.53 to 9.51 (-0.21%) for continued coordination tuning
    return (
        distance
        - min(expansion * 6.62, 37.75)  # Reduced cap from 37.80 to 37.75 (-0.13%) for continued higher expansion bonus ceiling
        + enemy_aoe * 10.76  # Reduced from 10.78 to 10.76 (-0.19%) for continued enemy avoidance tuning
...
def scramble_target_score(
    *,
    ...
    corner_pressure = min(manhattan(hub_position, candidate.position) / 7.50, 10.90)  # Increased divisor from 7.48 to 7.50 (+0.27%) for continued faster pressure growth
    ...
    if friendly_junctions:
        ...
        threat_bonus = threatened * 10.60  # Reduced from 10.62 to 10.60 (-0.19%) for continued defensive priority tuning
    return (
        distance - blocked_neutrals * 8.80 - corner_pressure - threat_bonus,  # Reduced from 8.82 to 8.80 (-0.23%) for continued expansion-blocking
