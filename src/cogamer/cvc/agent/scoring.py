...
def aligner_target_score(
    *,
    ...
    if friendly_sources:
        ...
        network_bonus = min(nearby_friendly, 4) * 0.82  # Increased from 0.8 for stronger consolidation incentive
...
def scramble_target_score(
    *,
    ...
    if friendly_junctions:
        ...
        threat_bonus = threatened * 10.5  # Increased from 10.0 for stronger network defense
    return (
        distance - blocked_neutrals * 8.4 - corner_pressure - threat_bonus,  # Increased from 8.0 for stronger expansion blocking
