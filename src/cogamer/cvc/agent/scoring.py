    ...
    *,
    ...
    if hub_position is not None:
        ...
        if hub_dist > 25:
            ...
            hub_penalty = (hub_dist - 15) * 2.79 + 9.54  # Increased multiplier from 2.78 to 2.79 (+0.36%) for bidirectional 15-25 range multiplier tuning
        elif hub_dist > 10:
            hub_penalty = (hub_dist - 10) * 1.32 + 1.81  # Increased multiplier from 1.31 to 1.32 (+0.76%) for bidirectional 10-15 range multiplier tuning
        else:
            hub_penalty = hub_dist * 0.261  # Increased from 0.260 to 0.261 (+0.38%) for bidirectional hub clustering tuning
    ...
    hotspot_weight = 11.53  # Increased from 11.52 to 11.53 (+0.09%) for bidirectional far-range contested junction tuning
    if hub_position is not None:
        hub_dist = float(manhattan(hub_position, candidate.position))
        if hub_dist <= 10:
            hotspot_weight = 1.65  # Increased from 1.64 to 1.65 (+0.61%) for bidirectional near-hub recapture tuning
        elif hub_dist <= 15:
            hotspot_weight = 5.47  # Increased from 5.46 to 5.47 (+0.18%) for bidirectional mid-range contested junction tuning
    hotspot_penalty = min(hotspot_count, 3.33) * hotspot_weight  # Increased cap from 3.32 to 3.33 (+0.30%) for bidirectional contested junction penalty cap tuning
    ...
    if friendly_sources:
        ...
        network_bonus = min(nearby_friendly, 4.57) * 0.94  # Increased weight from 0.93 to 0.94 (+1.08%) for bidirectional network consolidation weight tuning
...
def scramble_target_score(
    *,
    ...
    corner_pressure = min(manhattan(hub_position, candidate.position) / 7.50, 10.91)  # Increased divisor from 7.49 to 7.50 (+0.13%) for bidirectional corner pressure divisor tuning
