...
def aligner_target_score(
    *,
    ...
    if hub_position is not None:
        ...
        if hub_dist > 25:
            hub_penalty = (hub_dist - 25) * 7.80 + 48.83  # Increased multiplier from 7.79 to 7.80 (+0.13%) for bidirectional far-range multiplier tuning
        elif hub_dist > 15:
            hub_penalty = (hub_dist - 15) * 2.80 + 9.54  # Increased multiplier from 2.79 to 2.80 (+0.36%) for bidirectional 15-25 range multiplier tuning
        elif hub_dist > 10:
            hub_penalty = (hub_dist - 10) * 1.33 + 1.82  # Increased base from 1.81 to 1.82 (+0.55%) for bidirectional 10-15 range base tuning
        else:
            hub_penalty = hub_dist * 0.263  # Increased from 0.262 to 0.263 (+0.38%) for bidirectional hub clustering tuning
    ...
    hotspot_penalty = min(hotspot_count, 3.34) * hotspot_weight  # Increased cap from 3.33 to 3.34 (+0.30%) for bidirectional contested junction penalty cap tuning
    ...
    teammate_penalty = 9.53 if teammate_closer else 0.0  # Increased from 9.52 to 9.53 (+0.11%) for bidirectional coordination tuning
    return (
        distance
        - min(expansion * 6.64, 37.77)  # Increased cap from 37.76 to 37.77 (+0.03%) for bidirectional expansion bonus cap tuning
        + enemy_aoe * 10.78  # Increased from 10.77 to 10.78 (+0.09%) for bidirectional enemy avoidance tuning
...
def scramble_target_score(
    *,
    ...
    corner_pressure = min(manhattan(hub_position, candidate.position) / 7.51, 10.92)  # Increased cap from 10.91 to 10.92 (+0.09%) for bidirectional corner pressure cap tuning
    ...
    if friendly_junctions:
        ...
        threat_bonus = threatened * 10.63  # Increased from 10.62 to 10.63 (+0.09%) for bidirectional defensive priority tuning
    return (
        distance - blocked_neutrals * 8.83 - corner_pressure - threat_bonus,  # Increased from 8.82 to 8.83 (+0.11%) for bidirectional expansion-blocking tuning
