...
def aligner_target_score(
    *,
    ...
    if hub_position is not None:
        ...
        if hub_dist > 25:
            hub_penalty = (hub_dist - 25) * 7.81 + 48.84  # Increased base from 48.83 to 48.84 (+0.02%) for continued far-range base tuning
        elif hub_dist > 15:
    ...
    if hub_position is not None:
        hub_dist = float(manhattan(hub_position, candidate.position))
        if hub_dist <= 10:
            hotspot_weight = 1.66  # Increased from 1.64 to 1.66 (+1.22%) for continued near-hub contested junction tuning
        elif hub_dist <= 15:
