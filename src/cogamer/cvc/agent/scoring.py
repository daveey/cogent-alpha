...
def aligner_target_score(
    *,
    ...
    if hub_position is not None:
        ...
        if hub_dist > 25:
            hub_penalty = (hub_dist - 25) * 7.79 + 48.82  # Reduced base from 48.83 to 48.82 (-0.02%) for continued far-range base tuning
        elif hub_dist > 15:
    ...
    hotspot_weight = 11.50  # Reduced from 11.52 to 11.50 (-0.17%) for continued far-range contested junction tuning
    if hub_position is not None:
        hub_dist = float(manhattan(hub_position, candidate.position))
        if hub_dist <= 10:
            hotspot_weight = 1.62  # Reduced from 1.64 to 1.62 (-1.22%) for continued near-hub recapture tuning
        elif hub_dist <= 15:
            hotspot_weight = 5.44  # Reduced from 5.46 to 5.44 (-0.37%) for continued mid-range contested junction tuning
    hotspot_penalty = min(hotspot_count, 3.30) * hotspot_weight  # Reduced cap from 3.32 to 3.30 (-0.60%) for continued contested junction penalty cap tuning
    ...
    if friendly_sources:
        ...
        network_bonus = min(nearby_friendly, 4.54) * 0.92  # Reduced weight from 0.93 to 0.92 (-1.08%) for continued network consolidation tuning
    teammate_penalty = 9.49 if teammate_closer else 0.0  # Reduced from 9.51 to 9.49 (-0.21%) for continued coordination tuning
    return (
        distance
        - min(expansion * 6.60, 37.70)  # Reduced cap from 37.75 to 37.70 (-0.13%) for continued higher expansion bonus ceiling
        + enemy_aoe * 10.74  # Reduced from 10.76 to 10.74 (-0.19%) for continued enemy avoidance tuning
