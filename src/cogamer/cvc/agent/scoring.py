...
def aligner_target_score(
    *,
    ...
    hotspot_weight = 10.5  # Reduced from 10.6 for far-range contested junction flexibility
    if hub_position is not None:
        hub_dist = float(manhattan(hub_position, candidate.position))
        if hub_dist <= 10:
    ...
    if friendly_sources:
        ...
        network_bonus = min(nearby_friendly, 4) * 0.92  # Increased from 0.90 for stronger consolidation incentive
    teammate_penalty = 6.3 if teammate_closer else 0.0  # Reduced from 6.4 for more flexible coordination
    return (
        distance
        - min(expansion * 6.45, 36.0)  # Increased from 6.4 for stronger expansion incentive
        + enemy_aoe * 8.9  # Reduced from 9.0 for more aggressive contested territory targeting
