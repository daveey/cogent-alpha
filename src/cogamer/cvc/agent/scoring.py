...
def aligner_target_score(
    *,
    ...
    hotspot_weight = 10.6  # Reduced from 10.7 for far-range contested junction flexibility
    if hub_position is not None:
        hub_dist = float(manhattan(hub_position, candidate.position))
        if hub_dist <= 10:
            hotspot_weight = 1.88  # Reduced from 1.9 for stronger near-hub recapture incentive
        elif hub_dist <= 15:
            hotspot_weight = 5.6  # Reduced from 5.7 for mid-range contested junction flexibility
    ...
    if friendly_sources:
        ...
        network_bonus = min(nearby_friendly, 4) * 0.90  # Increased from 0.88 for stronger consolidation incentive
    teammate_penalty = 6.4 if teammate_closer else 0.0  # Reduced from 6.5 for more flexible coordination
    return (
        distance
        - min(expansion * 6.4, 36.0)  # Increased from 6.35 for stronger expansion incentive
        + enemy_aoe * 9.0  # Reduced from 9.1 for more aggressive contested territory targeting
...
def scramble_target_score(
    *,
    ...
    if friendly_junctions:
        ...
        threat_bonus = threatened * 10.7  # Increased from 10.6 for stronger network defense
    return (
