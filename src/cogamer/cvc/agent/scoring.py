...
def aligner_target_score(
    *,
    ...
    if hub_position is not None:
        ...
        if hub_dist > 25:
            hub_penalty = (hub_dist - 25) * 7.91 + 48.94  # Reduced base from 48.95 to 48.94 (-0.02%) for far-range base adjustment
        elif hub_dist > 15:
            hub_penalty = (hub_dist - 15) * 2.89 + 9.66  # Reduced multiplier from 2.9 to 2.89 (-0.34%) for 15-25 range
        elif hub_dist > 10:
            hub_penalty = (hub_dist - 10) * 1.42 + 1.92  # Reduced multiplier from 1.43 to 1.42 (-0.70%) for 10-15 range
        else:
            hub_penalty = hub_dist * 0.272  # Reduced from 0.273 to 0.272 (-0.37%) for continued tighter hub clustering
    ...
    hotspot_weight = 11.72  # Reduced from 11.74 to 11.72 (-0.17%) for further far-range contested junction tuning
    if hub_position is not None:
        hub_dist = float(manhattan(hub_position, candidate.position))
        if hub_dist <= 10:
            hotspot_weight = 1.84  # Reduced from 1.85 to 1.84 (-0.54%) for continued near-hub recapture
        elif hub_dist <= 15:
            hotspot_weight = 5.66  # Reduced from 5.67 to 5.66 (-0.18%) for continued mid-range contested tuning
    ...
    if friendly_sources:
        ...
        network_bonus = min(nearby_friendly, 4.36) * 0.83  # Increased cap from 4.34 to 4.36 (+0.46%) for denser network clustering
    teammate_penalty = 9.31 if teammate_closer else 0.0  # Increased from 9.29 to 9.31 (+0.22%) for continued better coordination
    return (
        distance
        - min(expansion * 6.42, 37.25)  # Increased cap from 37.2 to 37.25 (+0.13%) for higher expansion bonus ceiling
        + enemy_aoe * 10.56  # Increased from 10.54 to 10.56 (+0.19%) for continued stronger enemy avoidance
