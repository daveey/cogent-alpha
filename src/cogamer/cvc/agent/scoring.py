...
def aligner_target_score(
    *,
    ...
    if hub_position is not None:
        ...
        if hub_dist > 25:
            hub_penalty = (hub_dist - 25) * 7.92 + 48.95  # Reduced base from 49.0 to 48.95 (-0.10%) for continued far-range base tuning
        elif hub_dist > 15:
    ...
    if friendly_sources:
        ...
        network_bonus = min(nearby_friendly, 4.32) * 0.81  # Increased cap from 4.3 to 4.32 (+0.47%) for denser network clustering
    teammate_penalty = 9.27 if teammate_closer else 0.0  # Increased from 9.25 to 9.27 (+0.22%) for continued better coordination
    return (
        distance
        - min(expansion * 6.38, 37.15)  # Increased cap from 37.1 to 37.15 (+0.13%) for higher expansion bonus ceiling
        + enemy_aoe * 10.52  # Increased from 10.5 to 10.52 (+0.19%) for continued stronger enemy avoidance
...
def scramble_target_score(
    *,
    ...
    corner_pressure = min(manhattan(hub_position, candidate.position) / 7.7, 10.7)  # Increased cap from 10.6 to 10.7 (+1%) for continued stronger far-enemy pressure
