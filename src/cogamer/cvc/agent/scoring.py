...
def aligner_target_score(
    *,
    ...
    if hub_position is not None:
        ...
        if hub_dist > 25:
            hub_penalty = (hub_dist - 25) * 7.87 + 48.90  # Reduced base from 48.91 to 48.90 (-0.02%) for continued far-range base tuning
        elif hub_dist > 15:
            hub_penalty = (hub_dist - 15) * 2.85 + 9.61  # Reduced multiplier from 2.86 to 2.85 (-0.35%) for continued 15-25 range multiplier tuning
        elif hub_dist > 10:
            hub_penalty = (hub_dist - 10) * 1.38 + 1.88  # Reduced multiplier from 1.39 to 1.38 (-0.72%) for continued 10-15 range multiplier tuning
        else:
            hub_penalty = hub_dist * 0.268  # Reduced from 0.269 to 0.268 (-0.37%) for continued tighter hub clustering tuning
    ...
    hotspot_weight = 11.62  # Reduced from 11.64 to 11.62 (-0.17%) for continued far-range contested junction tuning
    if hub_position is not None:
        hub_dist = float(manhattan(hub_position, candidate.position))
        if hub_dist <= 10:
            hotspot_weight = 1.74  # Reduced from 1.76 to 1.74 (-1.14%) for continued near-hub recapture tuning
        elif hub_dist <= 15:
            hotspot_weight = 5.56  # Reduced from 5.58 to 5.56 (-0.36%) for continued mid-range contested junction tuning
    hotspot_penalty = min(hotspot_count, 3.22) * hotspot_weight  # Increased cap from 3.20 to 3.22 (+0.63%) for continued contested junction penalty cap tuning
    ...
    if friendly_sources:
        ...
        network_bonus = min(nearby_friendly, 4.44) * 0.88  # Increased weight from 0.87 to 0.88 (+1.15%) for continued stronger network consolidation
    teammate_penalty = 9.41 if teammate_closer else 0.0  # Increased from 9.39 to 9.41 (+0.21%) for continued coordination tuning
    return (
        distance
        - min(expansion * 6.52, 37.50)  # Increased cap from 37.45 to 37.50 (+0.13%) for continued higher expansion bonus ceiling
        + enemy_aoe * 10.66  # Increased from 10.64 to 10.66 (+0.19%) for continued enemy avoidance tuning
