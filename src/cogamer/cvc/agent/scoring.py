    ...
    *,
    ...
    if hub_position is not None:
        ...
        if hub_dist > 25:
            ...
            hub_penalty = (hub_dist - 15) * 2.93 + 10.0  # Reduced from 2.95 for mid-far hub preference
        elif hub_dist > 10:
            hub_penalty = (hub_dist - 10) * 1.45 + 2.0  # Reduced from 1.47 for mid-range hub preference
        else:
            hub_penalty = hub_dist * 0.27  # Reduced from 0.28 for slightly stronger near-hub preference
    ...
    hotspot_weight = 10.7  # Reduced from 10.8 for far-range contested junction flexibility
    if hub_position is not None:
        hub_dist = float(manhattan(hub_position, candidate.position))
        if hub_dist <= 10:
            hotspot_weight = 1.88  # Reduced from 1.9 for stronger near-hub recapture incentive
        elif hub_dist <= 15:
            hotspot_weight = 5.7  # Reduced from 5.8 for mid-range contested junction flexibility
    ...
    if friendly_sources:
        ...
        network_bonus = min(nearby_friendly, 4) * 0.86  # Increased from 0.84 for stronger consolidation incentive
    teammate_penalty = 6.6 if teammate_closer else 0.0  # Reduced from 6.7 for more flexible coordination
    return (
        distance
        - min(expansion * 6.25, 36.0)  # Increased from 6.2 for stronger expansion incentive
        + enemy_aoe * 9.2  # Reduced from 9.3 for more aggressive contested territory targeting
...
def scramble_target_score(
    *,
    ...
    if friendly_junctions:
        ...
        threat_bonus = threatened * 10.6  # Increased from 10.5 for stronger network defense
    return (
        distance - blocked_neutrals * 8.5 - corner_pressure - threat_bonus,  # Increased from 8.4 for stronger expansion blocking
