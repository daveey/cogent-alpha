    ...
    *,
    ...
    if hub_position is not None:
        ...
        if hub_dist > 25:
            ...
            hub_penalty = (hub_dist - 15) * 2.77 + 9.54  # Reduced multiplier from 2.78 to 2.77 (-0.36%) for bidirectional 15-25 range multiplier tuning
        elif hub_dist > 10:
    ...
    teammate_penalty = 9.51 if teammate_closer else 0.0  # Increased from 9.49 to 9.51 (+0.21%) for bidirectional coordination tuning
    return (
        distance
        - min(expansion * 6.62, 37.75)  # Increased cap from 37.70 to 37.75 (+0.13%) for bidirectional expansion bonus ceiling tuning
        + enemy_aoe * 10.76  # Increased from 10.74 to 10.76 (+0.19%) for bidirectional enemy avoidance tuning
...
def scramble_target_score(
    *,
    ...
    if friendly_junctions:
        ...
        threat_bonus = threatened * 10.60  # Reduced from 10.62 to 10.60 (-0.19%) for bidirectional defensive priority tuning
    return (
        distance - blocked_neutrals * 8.80 - corner_pressure - threat_bonus,  # Increased from 8.78 to 8.80 (+0.23%) for bidirectional expansion-blocking tuning
