...
def aligner_target_score(
    *,
    ...
    if hub_position is not None:
        ...
        if hub_dist > 25:
            hub_penalty = (hub_dist - 25) * 7.82 + 48.85  # Reduced base from 48.86 to 48.85 (-0.02%) for continued far-range base tuning
        elif hub_dist > 15:
            hub_penalty = (hub_dist - 15) * 2.80 + 9.56  # Reduced multiplier from 2.81 to 2.80 (-0.36%) for continued 15-25 range multiplier tuning
        elif hub_dist > 10:
            hub_penalty = (hub_dist - 10) * 1.33 + 1.83  # Reduced multiplier from 1.34 to 1.33 (-0.75%) for continued 10-15 range multiplier tuning
        else:
            hub_penalty = hub_dist * 0.263  # Reduced from 0.264 to 0.263 (-0.38%) for continued tighter hub clustering tuning
    ...
    if hub_position is not None:
        ...
        if hub_dist <= 10:
            ...
            hotspot_weight = 5.46  # Reduced from 5.48 to 5.46 (-0.36%) for continued mid-range contested junction tuning
    hotspot_penalty = min(hotspot_count, 3.32) * hotspot_weight  # Increased cap from 3.30 to 3.32 (+0.61%) for continued contested junction penalty cap tuning
    ...
    if friendly_sources:
        ...
        network_bonus = min(nearby_friendly, 4.56) * 0.93  # Increased cap from 4.54 to 4.56 (+0.44%) for continued denser network clustering
    teammate_penalty = 9.51 if teammate_closer else 0.0  # Increased from 9.49 to 9.51 (+0.21%) for continued coordination tuning
    return (
        distance
...
def scramble_target_score(
    *,
    ...
    return (
        distance - blocked_neutrals * 8.78 - corner_pressure - threat_bonus,  # Increased from 8.76 to 8.78 (+0.23%) for continued expansion-blocking
