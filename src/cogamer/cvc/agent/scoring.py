...
def aligner_target_score(
    *,
    ...
    if hub_position is not None:
        ...
        if hub_dist > 25:
            hub_penalty = (hub_dist - 25) * 7.85 + 48.88  # Reduced base from 48.89 to 48.88 (-0.02%) for continued far-range base tuning
        elif hub_dist > 15:
            hub_penalty = (hub_dist - 15) * 2.83 + 9.59  # Reduced multiplier from 2.84 to 2.83 (-0.35%) for continued 15-25 range multiplier tuning
        elif hub_dist > 10:
            hub_penalty = (hub_dist - 10) * 1.36 + 1.86  # Reduced multiplier from 1.37 to 1.36 (-0.73%) for continued 10-15 range multiplier tuning
        else:
    ...
    hotspot_weight = 11.58  # Reduced from 11.60 to 11.58 (-0.17%) for continued far-range contested junction tuning
    if hub_position is not None:
        hub_dist = float(manhattan(hub_position, candidate.position))
        if hub_dist <= 10:
            hotspot_weight = 1.70  # Reduced from 1.72 to 1.70 (-1.16%) for continued near-hub recapture tuning
        elif hub_dist <= 15:
            hotspot_weight = 5.52  # Reduced from 5.54 to 5.52 (-0.36%) for continued mid-range contested junction tuning
    hotspot_penalty = min(hotspot_count, 3.24) * hotspot_weight  # Increased cap from 3.22 to 3.24 (+0.62%) for continued contested junction penalty cap tuning
    ...
    if friendly_sources:
        ...
        network_bonus = min(nearby_friendly, 4.48) * 0.89  # Increased cap from 4.46 to 4.48 (+0.45%) for continued denser network clustering
    teammate_penalty = 9.43 if teammate_closer else 0.0  # Increased from 9.41 to 9.43 (+0.21%) for continued coordination tuning
    return (
        distance
        - min(expansion * 6.54, 37.55)  # Increased cap from 37.50 to 37.55 (+0.13%) for continued higher expansion bonus ceiling
        + enemy_aoe * 10.68  # Increased from 10.66 to 10.68 (+0.19%) for continued enemy avoidance tuning
...
def scramble_target_score(
    *,
    ...
    corner_pressure = min(manhattan(hub_position, candidate.position) / 7.58, 10.82)  # Reduced divisor from 7.60 to 7.58 (-0.26%) for continued faster pressure growth
    ...
    if friendly_junctions:
        ...
        threat_bonus = threatened * 10.52  # Increased from 10.50 to 10.52 (+0.19%) for continued defensive priority tuning
    return (
        distance - blocked_neutrals * 8.72 - corner_pressure - threat_bonus,  # Increased from 8.70 to 8.72 (+0.23%) for continued expansion-blocking
